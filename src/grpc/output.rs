...
fn vec_u8_to_vec_16bits(data: Vec<u8>) -> Vec<f32> {
...
impl ModelOutput {
    pub fn new(response: ModelInferResponse) -> Self {
        Self::from(response)
    }
...
impl From<ModelInferResponse> for ModelOutput {
    fn from(response: ModelInferResponse) -> Self {
                ...
                TritonDataTypes::BOOL => {
                    let array = ArrayD::from_shape_vec(
                        output
                            ...
                            .map(|v| v as usize)
                            .collect::<Vec<_>>(),
                        vec_u8_to_vec_t::<bool>(raw_content),
                    )
                    .unwrap();
                ...
                TritonDataTypes::INT8 => {
                    let array = ArrayD::from_shape_vec(
                        output
                            ...
                            .map(|v| v as usize)
                            .collect::<Vec<_>>(),
                        vec_u8_to_vec_t::<i8>(raw_content),
                    )
                    .unwrap();
                ...
                TritonDataTypes::INT16 => {
                    let array = ArrayD::from_shape_vec(
                        output
                            ...
                            .map(|v| v as usize)
                            .collect::<Vec<_>>(),
                        vec_u8_to_vec_t::<i16>(raw_content),
                    )
                    .unwrap();
                ...
                TritonDataTypes::INT32 => {
                    let array = ArrayD::from_shape_vec(
                        output
                            ...
                            .map(|v| v as usize)
                            .collect::<Vec<_>>(),
                        vec_u8_to_vec_t::<i32>(raw_content),
                    )
                    .unwrap();
                ...
                TritonDataTypes::INT64 => {
                    let array = ArrayD::from_shape_vec(
                        output
                            ...
                            .map(|v| v as usize)
                            .collect::<Vec<_>>(),
                        vec_u8_to_vec_t::<i64>(raw_content),
                    )
                    .unwrap();
                ...
                TritonDataTypes::UINT8 => {
                    let array = ArrayD::from_shape_vec(
                        output
                            ...
                            .map(|v| v as usize)
                            .collect::<Vec<_>>(),
                        vec_u8_to_vec_t::<u8>(raw_content),
                    )
                    .unwrap();
                ...
                TritonDataTypes::UINT16 => {
                    let array = ArrayD::from_shape_vec(
                        output
                            ...
                            .map(|v| v as usize)
                            .collect::<Vec<_>>(),
                        vec_u8_to_vec_t::<u16>(raw_content),
                    )
                    .unwrap();
                ...
                TritonDataTypes::UINT32 => {
                    let array = ArrayD::from_shape_vec(
                        output
                            ...
                            .map(|v| v as usize)
                            .collect::<Vec<_>>(),
                        vec_u8_to_vec_t::<u32>(raw_content),
                    )
                    .unwrap();
                ...
                TritonDataTypes::UINT64 => {
                    let array = ArrayD::from_shape_vec(
                        output
                            ...
                            .map(|v| v as usize)
                            .collect::<Vec<_>>(),
                        vec_u8_to_vec_t::<u64>(raw_content),
                    )
                    .unwrap();
                ...
                TritonDataTypes::FP16 | TritonDataTypes::BF16 => {
                    let array = ArrayD::from_shape_vec(
                        output
                            ...
                            .map(|v| v as usize)
                            .collect::<Vec<_>>(),
                        vec_u8_to_vec_16bits(raw_content),
                    )
                    .unwrap();
                ...
                TritonDataTypes::FP32 => {
                    let array = ArrayD::from_shape_vec(
                        output
                            ...
                            .map(|v| v as usize)
                            .collect::<Vec<_>>(),
                        vec_u8_to_vec_t::<f32>(raw_content),
                    )
                    .unwrap();
                    inner.insert(output.name, ArrayOutputOneOf::FP32(array));
                }
                TritonDataTypes::FP64 => {
                    let array = ArrayD::from_shape_vec(
                        output
                            ...
                            .map(|v| v as usize)
                            .collect::<Vec<_>>(),
                        vec_u8_to_vec_t::<f64>(raw_content),
                    )
                    .unwrap();
